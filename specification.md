...
4. args[0]: 任意の型の入力を受け付ける
  - 入力ノードを接続すると、1行のテキスト入力がウィジェットに追加される
    - プレースホルダーは"The name of the keyword argument (or the positional argument if left blank)"
    - 文字列が入力されると、args[0]という名前が入力文字列に変わる。後続の入力の名前もargs[1] -> args[0]のように整合性を保つ
